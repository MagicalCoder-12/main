...
powerup_images = [
    ...
    os.path.join(current_dir,'Assets','Items','Fruits','Bananas.png'),
...
def main():
    ...
    while running:
        ...
        screen.blit(text, (10, 10))
         # Display timer counter for Bananas power-up
        if pygame.time.get_ticks() <= double_bonus:
                ...
                if check_collision_lasers_asteroid([laser], asteroid['x'], asteroid['y']):
                    ...
                    for powerup_type, probability in powerup_probabilities.items():
                        if random.random() <= probability:
                            ...
                            random_powerup_image = os.path.join("Assets", "Items", "Fruits", f"{powerup_type.capitalize()}.png")
