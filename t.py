...
def main():
    ...
    while running:
        ...
        if pygame.time.get_ticks() <= double_bonus:
            ...
            timer_text = font.render(f'Double Score: {remaining_time}s', True, (0, 0, 0))
        ...
        for asteroid in asteroids:
            if check_collision_player_asteroid(player, asteroid['x'], asteroid['y']):
                if pygame.time.get_ticks() - player.invincible_start_time >= 10000 or player.invincible_start_time == 0:
